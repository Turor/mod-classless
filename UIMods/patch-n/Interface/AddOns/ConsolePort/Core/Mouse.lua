...
function Camera:OnPlayerMove() if db('Mouse/Events/PLAYER_STARTED_MOVING') and not isTargeting and not hasItem then Camera:Start() end end
...
function Camera:OnUpdate(elapsed)
	...
	---------------
	CPAPI.SetShown(self.BlockUI,cameraMode)
	---------------
...
function Camera:OnInteract()
	...
	if guid then
		...
		canInteract = CheckInteractDistance('target', 3) or GetCVar('autoInteract') == '1' 
	end
	if canInteract then
		blockCursor = true
		Camera:Start()
...
for func, hook in pairs({
	...
	-- Get rid of mouselook when moving
	MoveForwardStart = Camera.OnPlayerMove,
	MoveBackwardStart = Camera.OnPlayerMove,
	StrafeLeftStart = Camera.OnPlayerMove,
	StrafeRightStart = Camera.OnPlayerMove,
	TurnLeftStart = Camera.OnPlayerMove,
	TurnRightStart = Camera.OnPlayerMove,
...
for name, script in pairs({
	_onattributechanged = [[
		if name == 'state-targetstate' then
			control:RunFor(self, self:GetAttribute('UpdateTarget'), value)
		elseif name == 'state-vehicle' then
			control:RunFor(self, self:GetAttribute('UpdateVehicle'), value)
		elseif name == 'blockhandle' then
			isEnabled = not value
			if not isEnabled then
				self:ClearBindings()
				control:CallMethod('TrackUnit', nil, true)
			else
				control:RunFor(self, self:GetAttribute('UpdateTarget'), target)
			end
		end
	]],
	UpdateVehicle = [[
	...
	UpdateTarget = [[
		...
		if (( interact or loot or npc ) and USEKEY) or (loot and LOOTKEY) then
			...
			control:RunFor(self, self:GetAttribute('Set'), USEKEY, ( (loot or npc) and 'INTERACTTARGET' or 'TURNORACTION' ) )  
		end
		control:CallMethod('CallMethodFromFrame', self:GetName(), 'TrackUnit', target)
	]],
...
Mouse.CallMethodFromFrame = CPAPI.CallMethodFromFrame
--
---------------------------------------------------------------
-- Display functions
---------------------------------------------------------------
...
function Mouse:SetHintAnchors(plate)
	...
	CPAPI.SetShown(self.Text, not displayOnPlate and true)
	...
	CPAPI.SetShown(self.Line, displayOnPlate and true or not (db'interactHintNoLine'))
...
function Trail:ShowPrimaryTextures(toggled)
	CPAPI.SetShown(self.Ghost,not toggled)
	CPAPI.SetShown(self.Use,toggled)
	CPAPI.SetShown(self.Cancel,toggled)
	CPAPI.SetShown(self.Primary,toggled)
	CPAPI.SetShown(self.Secondary,toggled)
end
...
function Core:UpdateMouseDriver()
	if not InCombatLockdown() then
		...
		if button then
			...
			RegisterStateDriver(Mouse, 'vehicle', '[vehicleui] true; nil')
		...
		CPAPI.SetShown(ConsolePortTargetAI, loot or button and db('interactCache'))
		------------------------------------------------
		Trail.ghostMode = db('cursorTrailGhost')
		Trail.ghostAlpha = db('cursorTrailGhostVis')
		Trail.Ghost:SetTexture(db('cursorTrailGhostTex'))
		CPAPI.SetShown(Trail.Ghost, db('cursorTrailGhost'))
		Trail:SetScript('OnUpdate', Trail.OnUpdate)
		CPAPI.SetShown(Trail,not db('disableCursorTrail'))
		------------------------------------------------
